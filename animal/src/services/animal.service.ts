...
export class AnimalService {
    ...
    async createAnimal(createAnimalDto: CreateAnimalDto) {
        ...
        if (gender !== 1 && gender !== 2) {
            ...
            throw new BadRequestException("Le champ 'gender' doit être 1 (mâle) ou 2 (femelle)");
        }
    ...
    async createCategory(name: string) {
        ...
        this.logger.debug(`Category created successfully: ${JSON.stringify(result)}`);
        return result;
    }
    ...
    async getAnimalById(id: string) {
        ...
        const animal = await this.animalModel.findById(id).populate('categoryId').exec();
        if (!animal) {
    ...
    async updateAnimal(id: string, updateData: Partial<CreateAnimalDto>) {
        ...
        const updatedAnimal = await this.animalModel.findByIdAndUpdate(id, updateData, { new: true }).exec();
        if (!updatedAnimal) {
            this.logger.error(`Animal not found with ID: ${id} for update`);
            throw new NotFoundException("L'animal n'existe pas");
        }
        this.logger.debug(`Animal updated successfully: ${JSON.stringify(updatedAnimal)}`);
    ...
    async deleteAnimal(id: string) {
        ...
        this.logger.debug(`Animal deleted successfully: ${JSON.stringify(deletedAnimal)}`);
